...
//! 1. File at path `$NEMO_CLUSTER_CONFIG` (K8s ConfigMap mounted as a file)
...
impl ClusterConfig {
    ...
    pub fn load() -> Result<Self, ConfigLoadError> {
        // 1. Try file at $NEMO_CLUSTER_CONFIG
        if let Ok(path) = std::env::var("NEMO_CLUSTER_CONFIG") {
    ...
    fn from_env() -> Result<Self, ConfigLoadError> {
        let domain =
            std::env::var("NEMO_CLUSTER_DOMAIN").unwrap_or_else(|_| "localhost".to_string());

        Ok(Self {
            node_size: std::env::var("NEMO_CLUSTER_NODE_SIZE").ok(),
            provider: std::env::var("NEMO_CLUSTER_PROVIDER").ok(),
            domain,
            default_implementor: std::env::var("NEMO_CLUSTER_DEFAULT_IMPLEMENTOR").ok(),
            default_reviewer: std::env::var("NEMO_CLUSTER_DEFAULT_REVIEWER").ok(),
            max_parallel_loops_cap: std::env::var("NEMO_CLUSTER_MAX_PARALLEL_LOOPS_CAP")
                .ok()
                .and_then(|v| v.parse().ok()),
            max_cluster_jobs: std::env::var("NEMO_CLUSTER_MAX_CLUSTER_JOBS")
                .ok()
                .and_then(|v| v.parse().ok()),
        })
    }
    ...
    fn merge_with_env(mut config: Self) -> Self {
        if config.node_size.is_none() {
            config.node_size = std::env::var("NEMO_CLUSTER_NODE_SIZE").ok();
        }
        if config.provider.is_none() {
            config.provider = std::env::var("NEMO_CLUSTER_PROVIDER").ok();
        }
        if config.default_implementor.is_none() {
            config.default_implementor = std::env::var("NEMO_CLUSTER_DEFAULT_IMPLEMENTOR").ok();
        }
        if config.default_reviewer.is_none() {
            config.default_reviewer = std::env::var("NEMO_CLUSTER_DEFAULT_REVIEWER").ok();
        }
        if config.max_parallel_loops_cap.is_none() {
            config.max_parallel_loops_cap = std::env::var("NEMO_CLUSTER_MAX_PARALLEL_LOOPS_CAP")
                .ok()
                .and_then(|v| v.parse().ok());
        }
        if config.max_cluster_jobs.is_none() {
            config.max_cluster_jobs = std::env::var("NEMO_CLUSTER_MAX_CLUSTER_JOBS")
                .ok()
                .and_then(|v| v.parse().ok());
        }
        config
...
mod tests {
    ...
    fn test_cluster_file_deserialize() {
        ...
        assert_eq!(file.cluster.domain, "nemo.example.com");

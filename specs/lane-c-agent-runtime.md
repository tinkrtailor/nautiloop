...
- FR-22: The auth-sidecar runs as a **K8s native sidecar** — i.e. an `initContainer` with `restartPolicy: Always` (k8s 1.29+ GA, supported on the k3s versions nautiloop pins). This gives us:
  1. **Ordering for free:** the sidecar's `startupProbe` on `:9093/healthz` gates the agent container start. The agent does not run until the sidecar reports ready.
  2. **Lifecycle coupling:** when the agent container exits, k8s auto-terminates the sidecar via SIGTERM, the pod reaches `Succeeded`, and the Job reaches `Complete`. Without this the long-running sidecar would outlive the agent forever and every successful run would only terminate via `activeDeadlineSeconds` as a `DeadlineExceeded` failure (issue #53).
  3. **No mid-job restarts:** the sidecar deliberately has NO `livenessProbe`. For a Job pod, kubelet-restarting a sidecar mid-flight is strictly harmful — in-flight model API connections die, the agent has no recovery path, and the result is silent corruption. If the sidecar truly dies, the agent will encounter connection refused on localhost proxy ports; the entrypoint shall detect this and exit with code 111 (sidecar connection failure) so the control plane can distinguish sidecar crashes from agent failures.

  The sidecar additionally writes a readiness file to `/tmp/shared/ready` (shared emptyDir volume) on startup as a defense-in-depth signal for the agent entrypoint, but the primary ordering guarantee is now the k8s native sidecar `startupProbe`.
...
- FR-30: Sidecar startup ordering is enforced by the K8s native sidecar pattern (FR-22): the sidecar lives in `initContainers` with `restartPolicy: Always`, and its `startupProbe` on `:9093/healthz` gates the agent container start. As a defense-in-depth signal, the sidecar also writes `/tmp/shared/ready` (shared emptyDir volume) on startup, and the agent entrypoint polls this file (100ms interval, 30s timeout) before proceeding. `shareProcessNamespace` shall NOT be used (it leaks `/proc` across containers).
